...
# 解析符号链接，确保能找到同目录的 TUI 模块（clashtool_tui.sh）
...
# 参数: $1 - 模块文件名 (如 clashtool_tui.sh / clashtool_i18n.sh)
...
# Source TUI module（本地优先，本地缺失则在线加载）
_load_module "clashtool_tui.sh"
...
update_script(){
    ...
    if [ -f "$_us_tui_temp" ] && grep -q '^#' "$_us_tui_temp" 2>/dev/null; then
        mv "$_us_tui_temp" "${current_path%/*}/clashtool_tui.sh"
        chmod 755 "${current_path%/*}/clashtool_tui.sh"
    else
...
# select_subscription moved to clashtool_tui.sh
...
# logs_menu moved to clashtool_tui.sh
...
# TUI detection and rendering moved to clashtool_tui.sh
...
# main menu and sub-menus moved to clashtool_tui.sh
